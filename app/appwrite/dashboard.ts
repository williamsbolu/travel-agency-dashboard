...
export const getUsersAndTripsStats = async (): Promise<DashboardStats> => {
  ...
  const startCurrent = new Date(d.getFullYear(), d.getMonth(), 1).toISOString(); // start of the current month
  const startPrev = new Date(
    d.getFullYear(),
    d.getMonth() - 1,
    1
  ).toISOString(); // start of the last month
  const endPrev = new Date(d.getFullYear(), d.getMonth(), 0).toISOString(); // end of the last month
  ...
  // Ex: Tells us how many users has joined during the provided start and the end date: filters an array of items based on a date range
  ...
  return {
    ...
    userRole: {
      ...
      // How many user have joined within a specific period of time that has the role of user
